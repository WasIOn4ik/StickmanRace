...
namespace YG
{
	public class LeaderboardYG : MonoBehaviour
	{
		...
		{
			var layout = transform.GetComponentInChildren<VerticalLayoutGroup>();
			GameObject sampleContainer = layout.transform.GetChild(0).gameObject;

			Debug.Log("Обработка лидерборда " + _name);
			...
			else if (_name == nameLB)
			{
				...
				else
				{
					...
					if (entriesLB == "No data")
					{
						...
						Debug.Log($"Лидерборд пуст, поэтому скрыт {_name}");
					}
					else
					{
						Debug.Log($"Лидерборд не пуст, заполнение {_name}");
